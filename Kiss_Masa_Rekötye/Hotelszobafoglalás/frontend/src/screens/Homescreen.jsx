  ...
  useEffect(() => {
    ...
    if (extras.length > 0) {
      let filteredRooms = duplicaterooms.filter((room) => {
        // Ha a "Nem kérek semmit" van kiválasztva, minden szobát mutatunk
        if (extras.includes("Nem kérek semmit")) {
          return true;
        }
      ...
      setRooms(duplicaterooms); // Ha nincs extra kiválasztva, az összes szobát mutasd
    }
    ...
    <div className="container">
      ...
      <div className="search row">
        <div className="rangepickerdiv col-md-4">
          <RangePicker format="DD-MM-YYYY" onChange={filterByDate} placeholder={["ETTŐL", "EDDIG"]} />
        </div>
      ...
      <div className="room m-10">
        {loading ? (
          ...
          rooms.map((room) => (
            <div className="room-kontener" key={room._id}>
              <Room
                room={room}
                fromdate={fromdate}
                todate={todate}
